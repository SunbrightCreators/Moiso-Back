...
class UserSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        # id는 서버가 생성 → read_only, password는 write_only
        read_only_fields = ("id",)
        extra_kwargs = {
...
class UserLoginSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        user = authenticate(
            ...
            username=email,  # 커스텀 유저: USERNAME_FIELD = email
            password=password,
        )
        if not user:
...
class ProposerSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ("id", "user")  # ← id/user는 입력 불가
